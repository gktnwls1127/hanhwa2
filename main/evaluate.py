...
def scoring(truth_f, sub_f):
    ...
    for lt in truth_f:
        ...
        # Skip impressions with no relevant items (all labels are 0)
        if sum(labels) == 0:
            line_index += 1
            continue
        ...
        # Handle length mismatch: if sub_ranks is longer, truncate to truth length
        if len(y_score) > len(y_true):
            y_score = y_score[:len(y_true)]
        # If truth is longer, pad y_score with 0 (worst rank)
        elif len(y_true) > len(y_score):
            y_score.extend([0.0] * (len(y_true) - len(y_score)))
...
if __name__ == '__main__':
    ...
    if os.path.isdir(submit_dir) and os.path.isdir(truth_dir):
        ...
        auc, mrr, ndcg5, ndcg10 = scoring(truth_file, submission_answer_file)
